...
import { setListInsights } from '@redux/insights/slice';
...
const withInsights = ({ callback = () => null }: GsspType) =>
  wrapper.getStaticProps((store): any => async (ctx: GetServerSidePropsContext) => {
    ...
    const promises = [
      ...
      readCache(CACHE_GET_UPDATE_SECTION) ?? insightServices.getListInsights(reqUpdateSection),
    ];
    const response: any = await Promise.allSettled(promises);

    const [menuList, footerMenu, listInsights] = await response.map((item) =>
      item.status === 'fulfilled' ? item.value ?? [] : null,
    );

    store.dispatch(setMenuHeader(menuList));
    store.dispatch(setFooterMenu(footerMenu));
    store.dispatch(setListInsights(listInsights));
