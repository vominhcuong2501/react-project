...
import { submitFormGetInDataHubThunk, submitOtpGetInTouchThunk } from '@redux/app/thunks';
...
export function GroupGetInDataHub({ bannerImage }: GroupGetInTouchProps) {
  ...
  const handleSubmitForm = useDebouncedCallback(async (values) => {
    ...
    const response: any = await dispatch(submitFormGetInDataHubThunk(values));
...
GroupGetInDataHub.defaultProps = defaultProps;
